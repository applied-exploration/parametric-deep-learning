...
def generate_dataset(config: DataConfig, display_plot:bool=False):
    ...
    for i in range(config.num_circles):
        ...
        random_circle_radius = min(config.max_radius, random.random()*config.max_radius)
        random_translation_x = random.uniform(-1,1) * (config.canvas_size/2-config.max_radius)
        random_translation_y = random.uniform(-1,1) * (config.canvas_size/2-config.max_radius)
...
if __name__ == '__main__':
    dataconfig = DataConfig(canvas_size=100, max_radius=20, num_sample_points=100, num_circles=500)
