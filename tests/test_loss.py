...
def test_symbolic_loss() -> None:
    ...
    assert (
        -0.0001 <= symbolic_loss <= 0.0001
    ), "Symbolic loss should be 0.0, it was {}".format(symbolic_loss)


def test_label_loss() -> None:
    ...
    assert (
        -0.0001 <= label_loss <= 0.0001
    ), "Label loss should be 0.0, it was {}".format(label_loss)
